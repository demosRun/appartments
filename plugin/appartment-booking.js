...
function inventoryBooking(userID, value) {
  const startDate = getDateNDaysAgo(0); // Today
  const endDate = getDateNDaysAgo(31);  // 30 days from today

  fetch(`https://admin.booking.com/fresa/extranet/calendar/get_obp_advice_eligibility?lang=xu&hotel_id=${value.hotelID}&hotel_account_id=${pageData.partnerIdentity.partnerAccountId}&ses=${getQueryParam('ses')}`, {
    method: "GET",
    credentials: "include",
    headers: {
    ...
    .then(data => {
      ...
      console.log("Eligible Room IDs:", eligibleRoomIds);

      if (eligibleRoomIds.length === 0) {
        throw new Error("No eligible rooms found");
      }

      // Return eligible room IDs for the next .then() in the chain
      return { hotelId, roomIds: eligibleRoomIds };
    })
    .then(({ hotelId, roomIds }) => {
      console.log(`Fetching inventory for hotelId=${hotelId} with roomIds:`, roomIds);

      // Now fetch inventory data with the eligible room IDs
      return fetch(`https://admin.booking.com/fresa/extranet/inventory/fetch?lang=xu&hotel_id=${value.hotelID}&hotel_account_id=${pageData.partnerIdentity.partnerAccountId}&ses=${getQueryParam('ses')}`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
    ...
    .then(data => {
      ...
      throw new Error("Failed to process inventory data");
    })
    .catch(error => {
