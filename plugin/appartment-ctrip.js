...
function formatDateString(date) {
    ...
    // For YYYYMMDD strings
    if (typeof date === 'string' && date.length === 8 && !date.includes('-')) {
        return date.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3');
    }
    
    // For already formatted strings or other formats, return as is
    return date;
}
...
function initOwo() {
    window.owoSocket = new owoSocket('住宿管理', (meg) => {
        ...
        switch (meg.type) {
            ...
            //     break
            case "getInventoryCtrip":
                getInventoryCtrip(meg.userID, meg.value)
                break
            case "updateCtripRoom":
                updateCtripRoom(meg.userID, meg.value)
                break
        }
    }, true)
    window.runing = true

    // Listen for messages from parent window (inventory.html)
    window.addEventListener('message', function(event) {
        const meg = event.data;
        console.log('Received message from parent:', meg);
        
        // Process the message based on type
        if (meg && meg.type === 'updateCtripRoom') {
            updateCtripRoom('admin', meg.data);
        }
    });
}
...
function updateCtripRoom(userID, config) {
    // Ensure we have the full cipher information
    let cipherData = window.ctripCipherData || {};
    
    // If config has specific cipher information, use it instead
    if (config.cipher && Object.keys(config.cipher).length > 0) {
        cipherData = config.cipher;
    }
    ...
    const requestBody = {
        ...
        "basicRoomQuantityEditInfoList": [{
            "basicRoomId": config.basicRoomId,
            "startDate": config.startDate,
            "endDate": config.endDate,
            "canUsedQuantity": config.roomNum,
            "weekDayIndex": "1111111"
        }],
        "roomPriceEditInfoList": [{
            "hotelId": config.hotelId,
        ...
        "roomInventoryEditInfoList": [],
        "cipher": cipherData,
        "_hotelId": config.hotelId
    };
    
    console.log("Ctrip update request body:", requestBody);
    
    fetch('https://ebooking.ctrip.com/restapi/soa2/30535/setRoomInventoryInfo?_fxpcqlniredt=09031062412601353955&x-traceID=09031062412601353955-1746361204597-9762616', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json;charset=utf-8',
            'Accept': 'application/json',
